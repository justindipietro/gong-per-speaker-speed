...
// normName comes from norm.js, loaded before this script (see popup.html).
...
// Storage writes go through one chain so rapid clicks can't interleave
// read-modify-write cycles and drop an update.
...
function render(speakers, speedMap, defaultRate) {
  ...
  speakers.forEach((s) => {
    ...
    const row = makeStepperRow({
      ...
      onChange: (next) => queueWrite(async () => {
...
(async () => {
  ...
  console.log(LOG, "popup open", { tabId: tab?.id, speedMap, defaultRate });
  try {
    const resp = await chrome.tabs.sendMessage(tab.id, { type: "GET_SPEAKERS" });
