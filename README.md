...
| Keys | Action |
| --- | --- |
| `]` / `[` or `↑` / `↓` | Current speaker ±0.25× |
| `Shift + ]` / `Shift + [` or `Shift + ↑` / `Shift + ↓` | Global default ±0.25× |
...
Overrides are keyed by display name, so two different people who render with the same name share one rate.
...
| `norm.js` | Shared speaker-name normalization (loaded by content script and popup) |
...
The extension reads only the Gong call page DOM and stores per-speaker rate overrides in `chrome.storage.local` (your browser's local storage). It does not send data anywhere. Logs are kept in memory and only leave the browser when you click "Download logs" — note the downloaded files include speaker names, the call page URL, and your user agent, so treat them like any other call metadata before sharing.
